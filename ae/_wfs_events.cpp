...
static PyObject* _get_events(PyObject*self, PyObject* args, PyObject* kywds)
{
    py::object return_val;
    int exception_occured = 0;
    PyObject *py_local_dict = NULL;
    static const char *kwlist[] = {"callback","data","threshold","hdt","dead","stderr","local_dict", NULL};
    PyObject *py_callback, *py_data, *py_threshold, *py_hdt, *py_dead, *py_stderr;
    int callback_used, data_used, threshold_used, hdt_used, dead_used, stderr_used;
    py_callback = py_data = py_threshold = py_hdt = py_dead = py_stderr = NULL;
    callback_used= data_used= threshold_used= hdt_used= dead_used= stderr_used = 0;
    
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"OOOOOO|O:_get_events",const_cast<char**>(kwlist),&py_callback, &py_data, &py_threshold, &py_hdt, &py_dead, &py_stderr, &py_local_dict))
       return NULL;
    try                              
    {                                
        py_callback = py_callback;
        py::object callback = convert_to_catchall(py_callback,"callback");
        callback_used = 1;
        py_data = py_data;
        PyArrayObject* data_array = convert_to_numpy(py_data,"data");
        conversion_numpy_check_type(data_array,PyArray_SHORT,"data");
        #define DATA1(i) (*((short*)(data_array->data + (i)*Sdata[0])))
        #define DATA2(i,j) (*((short*)(data_array->data + (i)*Sdata[0] + (j)*Sdata[1])))
        #define DATA3(i,j,k) (*((short*)(data_array->data + (i)*Sdata[0] + (j)*Sdata[1] + (k)*Sdata[2])))
        #define DATA4(i,j,k,l) (*((short*)(data_array->data + (i)*Sdata[0] + (j)*Sdata[1] + (k)*Sdata[2] + (l)*Sdata[3])))
        npy_intp* Ndata = data_array->dimensions;
        npy_intp* Sdata = data_array->strides;
        int Ddata = data_array->nd;
        short* data = (short*) data_array->data;
        data_used = 1;
        py_threshold = py_threshold;
        int threshold = convert_to_int(py_threshold,"threshold");
        threshold_used = 1;
        py_hdt = py_hdt;
        int hdt = convert_to_int(py_hdt,"hdt");
        hdt_used = 1;
        py_dead = py_dead;
        int dead = convert_to_int(py_dead,"dead");
        dead_used = 1;
        py_stderr = py_stderr;
        FILE* stderr = convert_to_file(py_stderr,"stderr");
        stderr_used = 1;
        /*<function call here>*/     
...
{
    {"_get_events",(PyCFunction)_get_events , METH_VARARGS|METH_KEYWORDS},
    {NULL,      NULL}        /* Sentinel */
};

PyMODINIT_FUNC init_wfs_events(void)
{
    
    Py_Initialize();
    import_array();
    PyImport_ImportModule("numpy");
    (void) Py_InitModule("_wfs_events", compiled_methods);
}
